...
class RecordMachineAudioController:
    def __init__(self, model: RecordMachineAudioModel, view: RecordMachineAudioView, widget: QWidget,
                 analysis_result_emitter):
        self.model = model
        self.view = view
        self.widget = widget
    ...
    def record_audio(self):
        ...
        self.model.audio_manager.start_recording(self.model.ctx, self.model.selected_channels, self.model.sampling_rate,
                                                 self.model.channels)
        if self._plot_thread is None or not self._plot_thread.is_alive():
            self._plot_thread = threading.Thread(target=self.update_plot, args=(
            self.model.selected_channels, self.model.data_struct.audio_data, self.view.chart_graph), daemon=True)
    ...
    def init_new_canvas(self):
        x = np.linspace(-self.model.plot_points_section / self.model.sampling_rate, 0,
                        num=self.model.plot_points_section)
        self.view.chart_graph.clear()
    ...
    def start_analysis_process(self):
        ...
        try:
            ...
            self._analysis_proc = self._analysis_ctx.Process(target=analysis_worker,
